...
export function SheetCreatorWithLogs() {
  ...
  const createSheet = async () => {
    ...
    try {
      const response = await fetch("/api/create-sheet-with-logs", {
        method: "POST",
      })
      ...
      if (data.success) {
        ...
        setErrorMessage(data.error || "Failed to create Google Sheet")
      }
    } catch (error) {
      setCreationStatus("error")
  ...
  return (
    <Card className="w-full max-w-3xl mx-auto mt-8">
      <CardHeader>
        <CardTitle>Create New Google Sheet (With Detailed Logs)</CardTitle>
        <CardDescription>
          Create a new Google Sheet with the correct structure for the Honda Feedback System.
        </CardDescription>
      </CardHeader>
      <CardContent>
        <div className="space-y-4">
          ...
          {creationStatus === "error" && (
            <Alert className="mt-4 bg-red-50 border-red-200">
              ...
              <AlertDescription>
                <p>{errorMessage || "Failed to create Google Sheet. Please check your Google API credentials."}</p>
              </AlertDescription>
            </Alert>
          )}
