...
export function TestFixedCredentials() {
  ...
  const testCredentials = async () => {
    ...
    try {
      const response = await fetch("/api/test-with-fixed-credentials", {
        method: "POST",
        headers: {
  ...
  return (
    <Card className="w-full max-w-3xl mx-auto mt-8">
      <CardHeader>
        <CardTitle>Test Fixed Credentials</CardTitle>
        <CardDescription>
          Test if your fixed credentials can successfully connect to and write to your Google Sheet
        </CardDescription>
      </CardHeader>
      <CardContent>
        <div className="space-y-4">
          <p className="text-sm">
            Use this tool to test if your fixed credentials work properly before updating them in your Vercel project.
          </p>

          <div className="space-y-4">
            <div className="space-y-2">
              <Label htmlFor="fixed-credentials">Fixed Service Account Credentials (JSON)</Label>
              <Textarea
      ...
      <CardFooter>
        <p className="text-xs text-gray-500">
          Testing your fixed credentials before updating them in Vercel helps ensure a smooth transition.
        </p>
      </CardFooter>
    </Card>
  )
